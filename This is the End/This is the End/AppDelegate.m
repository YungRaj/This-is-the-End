...
{
    return
        [self topViewControllerWithRootViewController:[UIApplication sharedApplication].keyWindow.rootViewController];
}
...
- (UIViewController *)topViewControllerWithRootViewController:(UIViewController *)rootViewController
{
    if ([rootViewController isKindOfClass:[UINavigationController class]]) {
        ...
        return
            [rootViewController.childViewControllers objectAtIndex:[rootViewController.childViewControllers count] - 1];
    } else {
...
- (void)createWorkingPlistIntoDirectory:(NSSearchPathDirectory)directory
{
    ...
    NSString *writablePath = [[NSSearchPathForDirectoriesInDomains(directory, NSUserDomainMask, YES) firstObject]
        stringByAppendingString:@"GameContents.plist"];
...
- (void)applicationDidEnterBackground:(UIApplication *)application
{
    ...
    if ([topViewController isKindOfClass:[GameInstanceVC class]]) {
        [((SKView *)((GameInstanceVC *)topViewController).view).scene performSelector:@selector(pauseGame)
                                                                           withObject:nil];
    }
}
