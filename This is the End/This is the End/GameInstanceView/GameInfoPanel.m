...
-(void)setUpInfoPanel{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(updateValue:)
                                               ...
                                               object:nil];
    self.title.size = CGSizeMake(size.height*(self.title.texture.size.width/self.title.texture.size.height),
                                 size.height);
    self.title.position = CGPointMake(-self.size.width/2+self.title.size.width/2,0);
    self.value = [[SKNumberNode alloc]
                          initWithNumber:[self getValueFromStateWithType:self.type]
                                    size:CGSizeMake(size.width-self.title.size.width,size.height)
                                position:CGPointMake((((size.width-self.title.size.width)/2)+self.title.size.width)-self.size.width/2,0)
