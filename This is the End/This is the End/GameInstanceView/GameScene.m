...
- (instancetype)initWithSize:(CGSize)size gameStateData:(GameData *)state
{
    ...
    if (self) {
        ...
        CGSize buttonSize = CGSizeMake(size.width / 8, size.height / 6);
        CGSize childSize = CGSizeMake(buttonSize.width * .7, buttonSize.height * .7);
        CGSize pauseButtonSize = CGSizeMake(size.width / 12, size.height / 8);
        CGSize pauseSymbolSize = CGSizeMake(pauseButtonSize.width * 2 / 3, pauseButtonSize.height * 2 / 3);
        ...
        _left = [SKButton buttonWithTexture:squareButton withSize:buttonSize childTexture:arrow withSize:childSize];
        _right = [SKButton buttonWithTexture:squareButton withSize:buttonSize childTexture:arrow withSize:childSize];
        _fire = [SKButton buttonWithTexture:squareButton withSize:buttonSize childTexture:fireText withSize:childSize];
        _jump = [SKButton buttonWithTexture:squareButton withSize:buttonSize childTexture:jumpText withSize:childSize];
    }
...
- (void)didMoveToView:(SKView *)view
{
    ...
    if ([[rootViewController.childViewControllers objectAtIndex:[rootViewController.childViewControllers count] - 1]
...
- (void)setUpPhoneScene
{
    ...
    SKTexture *first = [SKTexture
        textureWithImageNamed:[[NSBundle mainBundle]
                                  pathForResource:[NSString stringWithFormat:@"background%d-1", self.state.worlds]
                                           ofType:@"png"]];

    SKTexture *second = [SKTexture
        textureWithImageNamed:[[NSBundle mainBundle]
                                  pathForResource:[NSString stringWithFormat:@"background%d-2", self.state.worlds]
                                           ofType:@"png"]];

    SKTexture *third = [SKTexture
        textureWithImageNamed:[[NSBundle mainBundle]
                                  pathForResource:[NSString stringWithFormat:@"background%d-3", self.state.worlds]
                                           ofType:@"png"]];

    for (int i = 0; i <= 21; i++) {
        SKSpriteNode *background = [[SKSpriteNode alloc] initWithTexture:nil color:nil size:self.size];
        switch (i % 3) {
            case 0:
                background.texture = first;
                break;
            case 1:
                background.texture = second;
                break;
            case 2:
                background.texture = third;
                break;
        }
    ...
    for (int i = 0; x <= self.level.size.width; i++) {
        ...
        switch (numPlatforms % 8) {
            case 3:
                enemy = [[Enemy alloc] initWithName:@"taserBot"
                                               size:CGSizeMake(self.size.width / 5, self.size.height / 5)
                                               type:EnemyAlien];
                enemy.canMove = YES;
                enemy.canAttack = YES;
                break;
            case 7:
                enemy = [[Enemy alloc] initWithName:@"missileBot"
                                               size:CGSizeMake(self.size.width / 6, self.size.height / 5)
                                               type:EnemyAlien];
                enemy.canMove = YES;
                enemy.canAttack = YES;
        ...
        if (numPlatforms % 4 == 3) {
            Block *mysteryBlock = [Block spriteNodeWithTexture:[SKTexture textureWithImageNamed:@"mysteryblock"]];
        ...
        SKSpriteNode *platform = [Platform
            spriteNodeWithTexture:[SKTexture textureWithImageNamed:[NSString stringWithFormat:@"platform%d",
                                                                                              self.state.worlds]]];

        platform.position =
            CGPointMake(x + platformFrame.size.width / 2, 0 - self.size.height / 8 - platformFrame.size.height / 2);
        platform.size = platformFrame.size;
        platform.physicsBody =
            [SKPhysicsBody bodyWithRectangleOfSize:CGSizeMake(platformFrame.size.width, platformFrame.size.height)];
    ...
    for (SKNode *node in self.level.children) {
        ...
        if (category == platformMask || category == blockMask) {
    ...
    self.currentLevel = [[GameInfoPanel alloc]
        initWithType:GameInfoPanelTypeLevel
            position:CGPointMake(self.size.width / 4 - self.size.width / 2, self.size.height / 2 - self.size.height / 8)
                size:CGSizeMake(self.size.width / 6, self.size.height / 24)
               state:self.state];
    ...
    self.coins = [[GameInfoPanel alloc]
        initWithType:GameInfoPanelTypeCoins
            position:CGPointMake(self.size.width / 2 - self.size.width / 5, self.size.height / 2 - self.size.height / 8)
    ...
    self.player.size = CGSizeMake(size.width / 4.5, size.height / 4);

    self.player.position = [self.level convertPoint:CGPointMake(0, .05) fromNode:self];
    ...
    self.pause.position = CGPointMake(size.width / 12 - size.width / 2, size.height / 1.1 - size.height / 2);
    self.left.position = CGPointMake(size.width / 1.5 - size.width / 2, size.height / 12 - size.height / 2);
    self.right.position = CGPointMake((size.width / 1.5) + self.left.size.width * 1.5 - self.size.width / 2,
                                      size.height / 12 - size.height / 2);
    self.fire.position = CGPointMake(size.width / 8 - size.width / 2, size.height / 12 - size.height / 2);
    self.jump.position = CGPointMake(size.width / 3.5 - size.width / 2, size.height / 12 - size.height / 2);
    ...
    self.player.physicsBody = [SKPhysicsBody
        bodyWithRectangleOfSize:CGSizeMake(self.player.size.width * .3, self.player.size.height)
                         center:[self.level convertPoint:CGPointMake(self.player.position.x, self.player.position.y)
                                                  toNode:self.player]];
...
{
    CGPoint cameraPositionInScene = [node.scene convertPoint:CGPointMake(node.position.x, node.position.y)
...
- (void)update:(NSTimeInterval)currentTime
{
    ...
    if (applicationState == UIApplicationStateBackground || applicationState == UIApplicationStateInactive) {
        [self pauseGame];
    }
...
- (void)didBeginContact:(SKPhysicsContact *)contact
{
    ...
    if (player && platform) {
        ...
        if (centerOfPlayer >= leftOfBlock && centerOfPlayer <= rightOfBlock && bottomOfPlayer < bottomOfBlock) {
