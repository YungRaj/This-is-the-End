...
- (void)viewDidLoad
{
    ...
    switch (self.mode) {
        case InterruptedGameOver:
            [self setUpGameOverMenu];
            break;
        case InterruptedGamePaused:
            [self setUpPauseMenu];
            break;
        case InterruptedGameLevelCompleted:
            break;
        case InterruptedGameWorldCompleted:
            break;
        default:
            break;
    }
...
- (void)setUpInterruptGame
{
    ...
    UIImage *border = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"heightborder"
                                                                                       ofType:@"png"]];

    CALayer *line1 = [CALayer layer];
    line1.contents = (id)border.CGImage;
    line1.frame = CGRectMake(0, 0, frame.size.width, frame.size.height / 15);
    ...
    line2.contents = (id)border.CGImage;
    line2.frame = CGRectMake(frame.size.width, 0, frame.size.height, frame.size.height / 15);
    ...
    line3.contents = (id)border.CGImage;
    line3.frame = CGRectMake(frame.size.width, frame.size.height, frame.size.width, frame.size.height / 15);
    ...
    line4.contents = (id)border.CGImage;
    line4.frame = CGRectMake(0, frame.size.height, frame.size.height, frame.size.height / 15);
    ...
    switch (self.mode) {
        case InterruptedGameOver:
            interruptionText =
                (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"gameover" ofType:@"png"]]
                    .CGImage;

            interruption.frame =
                CGRectMake(frame.size.width / 4, frame.size.height / 5, frame.size.width / 2, frame.size.height / 4);

            break;
        case InterruptedGamePaused:
            interruptionText =
                (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"paused" ofType:@"png"]]
                    .CGImage;

            interruption.frame =
                CGRectMake(frame.size.width / 3, frame.size.height / 4, frame.size.width / 3, frame.size.height / 6);
            break;
        default:
            break;
    }
...
- (void)setUpGameOverMenu
{
    ...
    UIImage *squareButton = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"squarebutton"
                                                                                             ofType:@"png"]];

    [self.continueGame setBackgroundImage:button forState:UIControlStateNormal];

    self.continueGame.frame =
        CGRectMake(frame.size.width / 6, frame.size.height / 1.8, frame.size.width / 5, frame.size.height / 6);
    ...
    retryText.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"retry" ofType:@"png"]].CGImage;
    retryText.frame = CGRectMake(continueGameButtonFrame.size.width / 8, continueGameButtonFrame.size.height / 3.5,
                                 continueGameButtonFrame.size.width / 2, continueGameButtonFrame.size.height / 2.4);

    [self.continueGame.layer addSublayer:retryText];
    CALayer *retrySymbol = [CALayer layer];

    retrySymbol.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"retrysymbol" ofType:@"png"]]
            .CGImage;
    retrySymbol.frame = CGRectMake(continueGameButtonFrame.size.width * .7, continueGameButtonFrame.size.height / 3,
                                   continueGameButtonFrame.size.width / 6, continueGameButtonFrame.size.height / 3);
    ...
    self.quit.frame = CGRectMake(frame.size.width / 6 + frame.size.width / 5 + frame.size.width / 12,
                                 frame.size.height / 1.8, frame.size.width / 5, frame.size.height / 6);
    [self.interruptView addSubview:self.quit];

    CALayer *quitText = [CALayer layer];

    quitText.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"quit" ofType:@"png"]].CGImage;
    quitText.frame = CGRectMake(self.quit.frame.size.width / 6, self.quit.frame.size.height / 4,
                                self.quit.frame.size.width / 1.5, self.quit.frame.size.height / 2);
    [self.quit.layer addSublayer:quitText];

    self.store.frame =
        CGRectMake(frame.size.width / 6 + frame.size.width * 2 / 5 + frame.size.width / 12 + frame.size.width / 12,
                   frame.size.height / 1.8, frame.size.width / 8, frame.size.height / 6);
    ...
    storeIcon.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"shopsymbol" ofType:@"png"]]
            .CGImage;
    storeIcon.frame = CGRectMake(storeFrame.size.width / 4, storeFrame.size.height / 4, storeFrame.size.width / 1.8,
...
- (void)setUpPauseMenu
{
    ...
    [self.continueGame setBackgroundImage:button forState:UIControlStateNormal];
    self.continueGame.frame =
        CGRectMake(frame.size.width / 12, frame.size.height / 2, frame.size.width / 5, frame.size.height / 6);
    ...
    playText.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"play" ofType:@"png"]].CGImage;
    playText.frame = CGRectMake(continueGameButtonFrame.size.width / 6, continueGameButtonFrame.size.height / 3.5,
                                continueGameButtonFrame.size.width / 2, continueGameButtonFrame.size.height / 2.5);
    ...
    playSymbol.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"playsymbol" ofType:@"png"]]
            .CGImage;
    playSymbol.frame = CGRectMake(continueGameButtonFrame.size.width * .7, continueGameButtonFrame.size.height / 3,
                                  continueGameButtonFrame.size.width / 4, continueGameButtonFrame.size.height / 3);

    [self.continueGame.layer addSublayer:playSymbol];

    self.saveAndQuit.frame =
        CGRectMake(frame.size.width * 3.5 / 10, frame.size.height / 2, frame.size.width / 3.34, frame.size.height / 6);
    ...
    saveAndQuitText.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"saveandquit" ofType:@"png"]]
            .CGImage;
    saveAndQuitText.frame = CGRectMake(saveAndQuitFrame.size.width / 12, saveAndQuitFrame.size.height / 4,
                                       saveAndQuitFrame.size.width / 1.2, saveAndQuitFrame.size.height / 2.2);
    [self.saveAndQuit.layer addSublayer:saveAndQuitText];

    self.quit.frame = CGRectMake(
        (saveAndQuitFrame.origin.x + saveAndQuitFrame.size.width +
         (saveAndQuitFrame.origin.x - (continueGameButtonFrame.origin.x + continueGameButtonFrame.size.width))),
        frame.size.height / 2, frame.size.width / 5, frame.size.height / 6);
    [self.quit setBackgroundImage:button forState:UIControlStateNormal];
    [self.interruptView addSubview:self.quit];
    ...
    quitText.contents =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"quit" ofType:@"png"]].CGImage;
    quitText.frame = CGRectMake(quitFrame.size.width / 6, quitFrame.size.height / 4, quitFrame.size.width / 1.5,
                                quitFrame.size.height / 2);
    [self.quit.layer addSublayer:quitText];

    self.store.frame =
        CGRectMake(frame.size.width / 2 - frame.size.width / 16, frame.size.height / 1.25 - frame.size.height / 12,
                   frame.size.width / 8, frame.size.height / 6);
    [self.store setBackgroundImage:squareButton forState:UIControlStateNormal];
    [self.interruptView addSubview:self.store];
...
- (void)tapView:(UITapGestureRecognizer *)gesture
{
    ...
    if (self.mode == InterruptedGamePaused) {
        ...
        if (CGRectContainsPoint(self.continueGame.frame, point)) {
            ...
            [((UIViewController *)[parentViewController.childViewControllers
                  objectAtIndex:0]).view removeFromSuperview];
            [((UIViewController *)[parentViewController.childViewControllers
                  objectAtIndex:1]).view removeFromSuperview];
...
- (void)loadGameMenu
{
    ...
    GameInstanceVC *gameInstance = (GameInstanceVC *)[self.parentViewController.childViewControllers objectAtIndex:0];
