...
- (instancetype)init
{
    self = [super
        initWithTexture:[SKTexture
                            textureWithImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle]
                                                                                  pathForResource:@"stormSix_standing_1"
                                                                                           ofType:@"png"]]]
...
+ (NSString *)filePath
{
    return [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject]
        stringByAppendingPathComponent:@"player"];
}
...
- (void)addAchievement:(Achievement *)achievment
{
    ...
    [GKAchievement reportAchievements:[NSArray arrayWithArray:self.achievements]
...
- (void)action
{
    ...
    if (isRightSelected && !isLeftSelected) {
        ...
        if (currentXScale != -1.0) {
            ...
            CGVector velocity = CGVectorMake(
                self.size.width * (18 * FRAME_RATE / 667) * xScaleNegativeDirectionFromIOSVersion(), playerVelocityY);
...
- (void)attack
{
    ...
    SKAction *bullet = [SKAction runBlock:^{
      Bullet *bullet = [[Bullet alloc] initWithName:@"player_bullet"
                                               size:CGSizeMake(self.size.height / 4, self.size.height / 6)];
      ...
      if (xScaleNegativeDirectionFromIOSVersion() == 1) {
          shootPoint = CGPointMake(self.position.x + self.size.width * .35, self.position.y);
      } else {
          shootPoint = CGPointMake(self.position.x + (self.xScale * self.size.width * .35), self.position.y);
      }

      shootPoint = [self.parent convertPoint:shootPoint toNode:self.parent.parent];
...
- (void)moveAndAttack
{
    ...
    for (int i = 1; i <= 1; i++) {
        SKTexture *texture =
            [SKTexture textureWithImageNamed:[NSString stringWithFormat:@"stormSix_walkP_shoot_%d", i]];
    ...
    SKAction *fireAndWalk5 =
        [SKAction animateWithTextures:@[ [SKTexture textureWithImageNamed:@"stormSix_walkP_shoot_8"] ]
                         timePerFrame:0.2];
    ...
    SKAction *fireAndWalk = [SKAction sequence:@[
        fireAndWalk1, bullet, fireAndWalk2, bullet, fireAndWalk3, bullet, fireAndWalk4, bullet, fireAndWalk5,
        fireAndWalk6
    ]];
...
- (void)death
{
    ...
    for (int i = 1; i <= 5; i++) {
        SKTexture *texture = [SKTexture textureWithImageNamed:[NSString stringWithFormat:@"stormSix_death_%d", i]];

        [deathTextures addObject:texture];
    }

    SKAction *deathAnimation = [SKAction animateWithTextures:[NSArray arrayWithArray:deathTextures] timePerFrame:0.2];
