...
- (void)setUpPhoneView
{
    ...
    frame = CGRectMake(size.width / 3, size.height / 16, size.width / 3, size.height / 8);
    ...
    frame = CGRectMake(size.width / 20, size.height / 16, size.width / 8, size.height / 8);
    ...
    self.store.frame = CGRectMake(frame.size.width - (frame.size.width / 12 + frame.size.height / 12),
                                  frame.size.height / 16, frame.size.width / 12, frame.size.height / 8);
    [self.store
        setBackgroundImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"squarebutton"
                                                                                            ofType:@"png"]]
    ...
    frame.size = CGSizeMake(screen.size.width, screen.size.height / (screen.size.width / (55 / 2.25)));
    ...
    frame.origin = CGPointMake(screen.size.width, screen.size.height);
...
{
    CGSize size = self.view.bounds.size;
    CGSize gameStateSize = CGSizeMake(size.width / 1.2, size.height / 7);
    CGRect frame;
    ...
    if ([self.gameStateViews count] == 5U) {
        ...
        frame.origin = CGPointMake(size.width / 12, size.height / 4.5);
...
- (void)tapView:(UITapGestureRecognizer *)recognizer
{
    ...
    for (NSUInteger i = 0; i < [self.gameStateViews count]; i++) {
        ...
        if (CGRectContainsPoint([stateView.play.superview convertRect:stateView.play.frame toView:self.view], point)) {
            ...
            [parentVC transitionFromViewController:self
                                  toViewController:loadingGameScreenVC
                                          duration:0.5
        ...
        } else if (CGRectContainsPoint([stateView.createGame.superview convertRect:stateView.createGame.frame
                                                                            toView:self.view],
                                       point)) {
