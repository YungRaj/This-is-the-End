...
{
    if(frameCount >= 21)frameCount = 0;
    
    if(UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad)
    {
        return [UIImage imageWithContentsOfFile:
            [[NSBundle mainBundle]pathForResource:
             [NSString stringWithFormat:@"ls-iPad%d",++frameCount] ofType:@"png"]];
    }
    
    return [UIImage imageWithContentsOfFile:
             [[NSBundle mainBundle]pathForResource:
              [NSString stringWithFormat:@"ls%d",++frameCount] ofType:@"png"]];
}
...
-(void)setUpPhoneView{
    ...
    self.updateFrame = [NSTimer scheduledTimerWithTimeInterval:4.0f/21 target:weakSelf selector:@selector(updateFrame:) userInfo:nil repeats:YES];
    
    [self.view addSubview:self.loadingScreen];
    
    [self.cancelButton setBackgroundImage:[UIImage imageNamed:@"button.png"] forState:UIControlStateNormal];
    self.cancelText.contents = (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"cancel" ofType:@"png"]].CGImage;
    ...
    frame = CGRectMake(size.width/2.5,
                      size.height/1.18,
                      size.width/6,
                      size.height/8);
...
-(void)startLoadingGame
{
    ...
    self.gameReadyCheck = [NSTimer scheduledTimerWithTimeInterval:5.0f target:weakSelf selector:@selector(checkIfGameIsReady:) userInfo:nil repeats:NO];
...
-(void)checkIfGameIsReady:(NSTimer*)timer
{
    ...
    } else if(timer.timeInterval==0.0f)
    {
        ...
        self.gameReadyCheck = [NSTimer scheduledTimerWithTimeInterval:2.0f target:weakSelf selector:@selector(checkIfGameIsReady:) userInfo:nil repeats:YES];
    }
}
...
-(void)tapView:(UITapGestureRecognizer*)recognizer
{
    ...
    if(CGRectContainsPoint(self.cancelButton.frame,point))
    {
        ...
        [parentVC transitionFromViewController:self toViewController:gameStateMenuVC duration:0.5 options:UIViewAnimationOptionTransitionFlipFromBottom animations:nil completion:^(BOOL didComplete){
