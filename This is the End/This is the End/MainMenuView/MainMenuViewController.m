...
- (void)viewDidLoad
{
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(pauseUpdateFrame:)
                                                 name:kMainMenuPauseFrame
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(playUpdateFrame:)
                                                 name:kMainMenuResumeFrame
                                               object:nil];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if (!self.updateFrame) {
        self.updateFrame = [NSTimer scheduledTimerWithTimeInterval:2.5f / 11
                                                            target:self
...
- (UIImage *)imageWithFrameCount
{
    ...
    return [UIImage
        imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:[NSString stringWithFormat:@"mm%d", ++frameCount]
                                                                ofType:@"png"]];
}
...
- (void)setUpPhoneView
{
    ...
    UIImage *playButton = [UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"playbutton"
                                                                                           ofType:@"png"]];
    ...
    id helpText =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"help" ofType:@"png"]].CGImage;
    id settingsText =
        (id)[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"settings" ofType:@"png"]].CGImage;
    ...
    [self.playButton setImage:playButton forState:UIControlStateNormal];
    [self.helpButton setImage:optionsButton forState:UIControlStateNormal];
    [self.settingsButton setImage:optionsButton forState:UIControlStateNormal];
    ...
    frame = CGRectMake(screen.size.width / 5, screen.size.height / 1.4, screen.size.width / 6, screen.size.height / 5);

    buttonSize = frame.size;
    self.playButton.frame = frame;

    frame = CGRectMake(buttonSize.width / 12, buttonSize.height / 2.5, buttonSize.width / 1.2, buttonSize.height / 2.5);

    self.playText.frame = frame;

    frame =
        CGRectMake(screen.size.width / 2.25, screen.size.height / 1.3, screen.size.width / 6, screen.size.height / 8);

    buttonSize = frame.size;
    self.helpButton.frame = frame;

    frame = CGRectMake(buttonSize.width / 8, buttonSize.height / 4.5, buttonSize.width / 1.3, buttonSize.height / 1.85);
    self.helpText.frame = frame;

    frame =
        CGRectMake(screen.size.width / 1.45, screen.size.height / 1.3, screen.size.width / 6, screen.size.height / 8);

    buttonSize = frame.size;
    self.settingsButton.frame = frame;

    frame = CGRectMake(buttonSize.width / 12, buttonSize.height / 4, buttonSize.width / 1.2, buttonSize.height / 1.9);
...
- (void)playUpdateFrame:(NSNotification *)notification
{
    if ([notification.name isEqualToString:kMainMenuResumeFrame]) {
        self.updateFrame = [NSTimer scheduledTimerWithTimeInterval:2.5 / 11
                                                            target:self
                                                          selector:@selector(updateFrame:)
                                                          userInfo:nil
                                                           repeats:YES];
